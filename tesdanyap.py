...
def handle_command(command):
    if command == "/help":
        help_text = (
            ...
            "<b>🛠️ Kalibrasi Sensor:</b>\n"
            "<code>/calTDS1:&lt;angka&gt;</code> - Kalibrasi TDS1 (ppm)\n"
            "<code>/calTDS2:&lt;angka&gt;</code> - Kalibrasi TDS2 (ppm)"
        )
        return help_text
    ...
    elif command.startswith("/calTDS1:"):
        try:
            tds_value = int(command.split(":")[1])
            mqtt_client.publish("smartgreengarden/sensor/kalibrasi/tds1", tds_value)
            return f"✅ Kalibrasi TDS1 dikirim ke perangkat: {tds_value} ppm"
        except:
            return "❌ Format salah. Gunakan: /calTDS1:<angka>"

    elif command.startswith("/calTDS2:"):
        try:
            tds_value = int(command.split(":")[1])
            mqtt_client.publish("smartgreengarden/sensor/kalibrasi/tds2", tds_value)
            return f"✅ Kalibrasi TDS2 dikirim ke perangkat: {tds_value} ppm"
        except:
            return "❌ Format salah. Gunakan: /calTDS2:<angka>"
    else:
